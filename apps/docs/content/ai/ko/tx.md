...
Tx는 낙관적 saga 라이브러리입니다. 여러 단계를 순서대로 실행하고, 실패 시 완료된 단계의 보상 함수를 역순으로 호출합니다.

Tx는 원격 API, IndexedDB, React 상태 사이의 원자적 commit을 제공하지 않습니다. 보상은 best-effort이며 보상 함수 자체도 실패할 수 있습니다.

주요 특징:

- 단계별 실행과 명시적인 보상(compensate) 함수로 역순 보상을 지원합니다
