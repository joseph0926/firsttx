...
| `initialData` | `T`                              | X    | -                   | 초기 데이터. 저장된 데이터가 없을 때 사용됩니다. `patch()` 사용 시 필수                           |
| `version`     | `number`                         | X    | -                   | 스키마 버전. 변경 시 기존 데이터를 삭제합니다. `initialData`가 있으면 그 값으로, 없으면 null 반환 |
...
`version`이 변경되면 기존 데이터는 삭제됩니다. `initialData`가 있으면 그 값으로 초기화되고, 없으면 `null`을 반환하여 서버에서 새 데이터를 가져올 수 있습니다.
