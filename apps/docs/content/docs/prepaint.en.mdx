...
export default defineConfig({
  plugins: [
    react(),
    firstTx({ overlay: true }), // recommended: keep cached DOM outside the React root
  ],
});
```
...
<Callout type="info" title="Overlay-related plugin options">
  ...
  Overlay mode is recommended for current releases because it keeps cached client DOM outside the React root. The legacy direct-restore path is documented below only for migration and debugging.
</Callout>
...
## 4. Legacy direct-restore hydration & root guard

The current legacy direct-restore path inserts the cached client DOM into the React root and attempts hydration. Cached client-rendered DOM is not a supported React hydration input, so this behavior is an implementation detail rather than a supported rendering contract. Use overlay mode for new integrations.

Because of **CSS-in-JS, random IDs, timestamps, non-deterministic rendering**, the legacy path can also produce hydration mismatches.

### 4-1. Hydration → clean render fallback

The current implementation handles this process as follows,
...
The current `installRootGuard` responds to structural changes as follows,
