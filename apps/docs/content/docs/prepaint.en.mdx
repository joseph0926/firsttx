...
<DocsAnchor id="installation-basic-integration" />

## 1. Installation & basic integration

<DocsAnchor id="install-packages" />
...
<DocsAnchor id="vite-plugin-setup" />
...
export default defineConfig({
  plugins: [react(), firstTx({ policy: { routes: ["/dashboard", "/cart"] } })],
});
```
...
- `inline` (`boolean`, default `false`): inline the boot script only when you intentionally manage its CSP hash
- `policy` (`PrepaintPolicy`, disabled by default): exact routes with optional TTL, byte limit, and CSS capture settings
- `minify` (`boolean`, default `true` in production): minify the boot script
- `injectTo` (`"head" | "head-prepend" | "body" | "body-prepend"`, default `"head-prepend"`): script injection position
- `nonce` (`string | (() => string)`): nonce embedded in generated output; static Vite output cannot create one per response
- `devFlagOverride` (`boolean`): override the development flag instead of following Vite mode
...
<DocsAnchor id="entry-point-createfirsttxroot" />
...
type Snapshot = {
  ...
  styles?: Array<{ type: "inline"; content: string } | { type: "external"; href: string; content?: string }>;
};
```

- Default max age: `MAX_SNAPSHOT_AGE = 7 days`

<DocsAnchor id="boot-flow" />
...
<DocsAnchor id="capture-pipeline-what-dom-is-stored" />
...
<DocsAnchor id="capture-triggers" />
...
<DocsAnchor id="dom-serialization-scrubbing" />
...
<DocsAnchor id="style-collection" />
...
<DocsAnchor id="devtools-error-model" />

## 5. DevTools & error model

<DocsAnchor id="devtools-events" />
...
<DocsAnchor id="error-hierarchy-overview" />
...
<DocsAnchor id="synergy-with-local-first-tx" />
...
<DocsAnchor id="recommended-settings-caveats" />
