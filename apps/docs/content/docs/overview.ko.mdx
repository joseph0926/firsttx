...
FirstTx는 React 기반 CSR 앱에서 **재방문 visual replay · 클라이언트 상태 보존 · 낙관적 UI 보상**을
...
- **Tx** - UI 업데이트와 서버 요청을 saga 형태의 순서로 실행하고, 실패 시 보상 롤백·재시도·타임아웃을 처리하는 **실행 레이어**
...
2. **Local-First**로 클라이언트 모델 snapshot을 보존하고,
...
1. **Prepaint**: SSR 런타임을 추가하지 않고 메인 앱 번들보다 먼저 visual snapshot을 replay
2. **동기 + 비동기 API**: React 렌더링용 `getCachedSnapshot()` + 비동기 작업용 `getSnapshot()`
3. **보상 패턴**: 롤백을 한 번만 선언하면, 실패 시 역순으로 자동 실행
...
### Prepaint - “부트 구간 visual snapshot replay”

- 마지막 화면을 DOM + 스타일 스냅샷으로 저장해 두었다가
- 다음 방문 시 **React 번들보다 먼저** 해당 화면을 복원합니다.
- 스냅샷은 임시 비상호작용 visual cache이며, 핸드오프 과정에서 React 앱이 제어권을 가져갑니다.
...
- “여러 API 호출과 로컬 상태 변경 중 실패하면 완료된 단계의 보상 작업을 실행하고 싶을 때”
...
<Callout type="info" title="각 레이어는 독립적으로도 사용할 수 있습니다">
  <ul>
    <li>
      Prepaint만 붙여 <strong>재방문 빈 화면 시간 단축</strong>에 집중할 수도 있고,
    </li>
    <li>
...
- React 18.2+ 기반이고, CSR 렌더링이 기본인 프로젝트
