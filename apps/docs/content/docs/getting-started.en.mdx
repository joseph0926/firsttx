...
export default defineConfig({
  plugins: [
    react(),
    firstTx({ overlay: true }), // injects the boot script and keeps cached DOM outside the React root
  ],
});
```
...
<Callout type="warning" title="Use overlay mode in current releases">
  The legacy direct-restore path may attempt to hydrate cached client-rendered DOM. That path is documented for migration and debugging, but it is not a supported rendering contract. Keep <code>overlay: true</code> for new integrations.
</Callout>
