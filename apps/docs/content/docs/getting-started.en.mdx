...
1. **Level 1 - Prepaint only**: replay the last visual snapshot during revisit boot
2. **Level 2 - Local-First**: add persisted client data
...
<DocsAnchor id="vite-plugin-prepaint-boot-script" />
...
<DocsAnchor id="replace-the-entry-point-createfirsttxroot" />

## 3. Replace the entry point (`createFirstTxRoot`) - ⭐ Level 1

In your app entry point, replace `ReactDOM.createRoot` with `createFirstTxRoot`. That entry point is usually `main.tsx`.
...
4. Finally mounts the React app and removes the temporary visual cache during handoff.
...
<Callout type="success" title="Quick check: is Prepaint working?">
  <ol>
    ...
    <li>If the last visual snapshot appears before the React app mounts, Prepaint is working. Record the timing under your target device and network profile.</li>
  </ol>
...
<DocsAnchor id="define-a-local-first-model-definemodel" />
...
<DocsAnchor id="use-the-sync-hook-in-a-component-usesyncedmodel" />
...
<DocsAnchor id="wrap-optimistic-updates-in-tx-optional" />
...
2. If the server request fails, `compensate` runs and attempts to return the UI to its previous state.
...
<DocsAnchor id="devtools-and-where-to-go-next" />
...
<DocsAnchor id="recap-what-you-have-now" />

## Recap: what you have now

If you followed steps 1-6, your app now provides,

1. **Boot-time visual snapshot replay** to reduce revisit blank time (Prepaint)
2. **IndexedDB-backed local data with TTL/multi-tab sync** (Local-First)
3. **Saga-like optimistic steps with retry/compensation/timeout** (Tx)
