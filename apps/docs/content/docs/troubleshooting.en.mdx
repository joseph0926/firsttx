...
- `useModel` synchronously returns the in-memory external-store snapshot, while IndexedDB loading is asynchronous. The data can therefore look empty on the very first render. Handle the initial `loading` state.
