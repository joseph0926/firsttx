...
Tx is an **execution layer that runs optimistic UI updates and server requests as a saga-like sequence and compensates completed steps on failure**.
...
<Callout type="info" title="When Tx is a good fit">
  <ul>
    ...
    <li>
      When you want completed API calls/local model updates to have explicit <strong>compensating actions</strong>
    </li>
    <li>When you want to inspect failure cases end-to-end in DevTools</li>
  </ul>
</Callout>

Tx does not provide an atomic commit across remote APIs, IndexedDB, and React state. Compensation is a best-effort action and can itself fail.

---

<DocsAnchor id="installation-the-simplest-usage-starttransaction" />
...
<DocsAnchor id="transaction-lifecycle-states" />
...
- While in `running`, **await the current step** before calling the next `run()`.
  Concurrent calls will throw a **`TransactionStateError`**.
...
<Callout type="warning" title="CompensationFailedError">
  <ul>
    ...
    <li>
      If any of these compensations fail, a <code>CompensationFailedError</code> is thrown, and the compensation errors are exposed in <code>failures</code> together with <code>completedSteps</code>. The original step error that triggered rollback is not stored on this error.
    </li>
    <li>
...
<DocsAnchor id="tx-run-and-retry" />
...
<DocsAnchor id="react-hook-usetx" />
...
<ApiTable
  ...
  items={[
    ...
    },
    {
      ...
      type: "boolean",
      defaultValue: "false",
      description: "Whether to automatically call <code>cancel()</code> when the component unmounts. Cancellation aborts the current step if it respects <code>AbortSignal</code>; otherwise in-flight work may continue, but subsequent state updates and callbacks for this transaction are ignored.",
    },
  ]}
/>
...
<ApiTable
  ...
  items={[
    ...
    },
    {
      name: "cancel",
      type: "() => void",
      description: "Cancels the current transaction. If the running step uses <code>AbortSignal</code>, in-flight work is aborted; otherwise it may keep running, but its result is ignored and no state updates/callbacks are applied.",
    },
    {
      name: "isPending",
      type: "boolean",
...
<Callout type="info" title="React integration details">
  <ul>
    ...
    <li>
      When <code>cancelOnUnmount</code> is <code>true</code>, unmounting the component calls <code>cancel()</code>. If the active step honors <code>AbortSignal</code>, it will be aborted; otherwise it may finish but its result is ignored.
    </li>
    <li>
...
- `CompensationFailedError`
  ...
  - Exposes the compensation errors in `failures` and the number of completed steps in `completedSteps`; it does not retain the original step error that triggered rollback.
...
<DocsAnchor id="2-devtools-events" />
...
<DocsAnchor id="using-tx-with-local-first-prepaint" />
...
- **Prepaint** - Replays the last visual snapshot during revisit boot to reduce blank time. ([GitHub][1])
- **Local-First** - Uses IndexedDB as the single source of truth so state survives reloads and offline usage. ([GitHub][1])
- **Tx** - Sits on top to run optimistic steps and compensate completed steps in reverse order on failure. ([GitHub][1])
