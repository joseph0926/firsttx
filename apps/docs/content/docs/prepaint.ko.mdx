...
export default defineConfig({
  plugins: [
    react(),
    firstTx({
      overlay: true,                // 권장: 캐시 DOM을 React 루트 밖에 유지
...
## 4. Legacy direct-restore 하이드레이션 & 루트 가드

현재 legacy direct-restore 경로는 캐시된 client DOM을 React 루트에 삽입한 뒤 하이드레이션을 시도합니다. client-rendered DOM은 React가 지원하는 하이드레이션 입력이 아니므로, 이 동작은 지원 계약이 아니라 현행 구현 세부입니다. 새 통합에서는 오버레이 모드를 사용하세요.

또한 **CSS‑in‑JS, 랜덤 ID, 타임스탬프, 비결정적 렌더링** 등으로 mismatch가 발생할 수 있습니다.

현행 구현은 이 과정을 다음과 같이 처리합니다.
...
<Callout type="info" title="어디서 확인하나요?">
  <ul>
    ...
    <li>그러면 캡처/복원/legacy 하이드레이션 에러/스토리지 에러가 타임라인으로 표시되어, 재방문 replay 경로를 추적할 수 있습니다.</li>
  </ul>
</Callout>
...
- **현재 릴리스에서는 오버레이를 기본으로 사용**
  - 새 통합은 `overlay: true`를 사용해 캐시된 client DOM과 React 루트를 분리하세요.
  - `overlayRoutes`는 일부 경로에서만 오버레이 표시 방식을 선택하는 옵션이며, 캡처 또는 복원 허용 목록이 아닙니다.
