...
- `policy` (`PrepaintPolicy`, 기본 비활성화): 정확한 route와 선택 TTL, byte 제한, CSS 저장 설정
- `minify` (`boolean`, 프로덕션 기본 `true`): 부트 스크립트 minify 여부
- `injectTo` (`"head" | "head-prepend" | "body" | "body-prepend"`, 기본 `"head-prepend"`): 스크립트 삽입 위치
- `nonce` (`string | (() => string)`): 생성 결과물에 포함할 nonce. 정적 Vite 출력에서는 응답별 nonce를 만들 수 없음
- `devFlagOverride` (`boolean`): Vite mode 대신 개발 모드 플래그를 직접 지정
...
<DocsAnchor id="overlay-rendering" />
