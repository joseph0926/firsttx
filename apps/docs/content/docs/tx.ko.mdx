...
<ApiTable
  ...
  items={[
    ...
    },
    {
      ...
      type: "boolean",
      defaultValue: "false",
      description: "컴포넌트 언마운트 시 자동으로 <code>cancel()</code>을 호출할지 여부입니다. 현재 구현에서는 네트워크 요청 자체를 중단하지 않고, 완료 후 상태 업데이트/콜백 실행만 막는 <strong>논리적 취소</strong>입니다.",
    },
  ]}
/>
...
<ApiTable
  ...
  items={[
    ...
    },
    {
      name: "cancel",
      type: "() => void",
      description: "현재 진행 중인 트랜잭션 결과를 UI에 반영하지 않도록 논리적으로 취소합니다. 이미 시작된 네트워크/타이머 작업은 계속 진행되지만, 완료 후 상태 업데이트와 콜백 실행은 무시됩니다.",
    },
    {
      name: "isPending",
      type: "boolean",
...
<Callout type="info" title="React 통합 디테일">
  <ul>
    ...
    <li>
      <code>cancelOnUnmount</code>가 <code>true</code>면 언마운트 시 자동으로 <code>cancel()</code>이 호출되어 이후 상태 업데이트와 콜백 실행만 막습니다. 이미 진행 중인 네트워크 요청 자체는 취소하지 않습니다.
    </li>
    <li>
