...
Tx는 **낙관적 UI 업데이트와 서버 요청을 saga 형태로 실행하고, 실패 시 완료된 단계를 역순으로 보상**하는 실행 레이어입니다.
...
<Callout type="info" title="Tx를 쓰면 좋은 상황">
  <ul>
    ...
    <li>
      완료된 API 호출/로컬 모델 변경에 명시적인 <strong>보상 작업</strong>을 두고 싶을 때
    </li>
    <li>실패 케이스까지 DevTools에서 한 번에 추적하고 싶을 때</li>
  </ul>
</Callout>

Tx는 원격 API, IndexedDB, React 상태 사이의 원자적 commit을 제공하지 않습니다. 보상 작업도 실패할 수 있으므로 best-effort 복구로 다뤄야 합니다.
...
<Callout type="warning" title="CompensationFailedError">
  <ul>
    ...
    <li>
      이 보상 중 하나라도 실패하면 <code>CompensationFailedError</code>가 던져지며, 보상 오류는 <code>failures</code>에, 완료한 단계 수는 <code>completedSteps</code>에 담깁니다. 롤백을 시작하게 한 원래 단계 오류는 이 에러에 저장되지 않습니다.
    </li>
    <li>
...
- `CompensationFailedError`
  ...
  - 보상 오류는 `failures`에, 완료한 단계 수는 `completedSteps`에 담기며 롤백을 시작하게 한 원래 단계 오류는 보존하지 않습니다.
...
- **Tx** - 이 위에서 낙관적 단계를 실행하고 실패 시 완료된 단계를 역순으로 보상합니다. ([GitHub][1])
