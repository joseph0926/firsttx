...
<ApiTable
  ...
  items={[
    ...
    },
    {
      ...
      description: (
        <>
          IndexedDB에 저장될 실제 키입니다. 따로 지정하지 않으면 <code>defineModel</code>의 첫 번째 인자(<code>name</code>)가 사용됩니다.
        </>
      ),
    },
  ]}
...
export function CartSidebar() {
  const [cart, patch, history, error] = useModel(CartModel);
...
이 정도만 지켜도, Local‑First를 **“그냥 쓰면 잘 돌아가는 로컬 퍼시스턴스 계층”**으로 안정적으로 활용할 수 있습니다.
