...
**persistent client cache**를 만드는 패키지입니다.

- IndexedDB를 단일 소스로 두고
- Zod 스키마로 타입·무결성을 검증하며
...
<Callout type="info" title="언제 Local-First를 쓰면 좋을까요?">
  <ul>
    ...
    <li>
      오프라인/불안정한 네트워크에서 저장된 <strong>“마지막으로 알던 상태”</strong>를 읽고 싶을 때
    </li>
...
<ApiTable
  ...
  items={[
    ...
    },
    {
      ...
      description: (
        <>
          스키마/데이터 구조가 깨질 수 있는 변경이 있을 때 올려주는 버전입니다. 디스크에 저장된 버전과 다르면 기존 데이터를 삭제합니다. <code>initialData</code>가 있으면 그 값으로 초기화되고, 없으면 <code>null</code>을 반환하여 서버에서 새 데이터를 가져올 수 있습니다.
        </>
      ),
    },
    {
...
<Callout type="warning" title="데이터 무결성 방어">
  <ul>
    ...
    <li>
      <code>version</code>을 올릴 때 <code>initialData</code>를 함께 정의해 두면 "깨진 데이터 → 새 초기 상태"로 자연스럽게 넘어갑니다. 없으면 <code>null</code>이 반환되어 서버에서 새 데이터를 가져올 수 있습니다.
    </li>
  </ul>
</Callout>
...
- `useModel`은 모델이 메모리에 보관한 현재 **external-store snapshot**을 동기적으로 반환합니다.
- IndexedDB 로드는 모델 수명주기에서 비동기로 진행되며, 완료되면 이 snapshot을 갱신합니다. 훅 자체는 서버 동기화를 시작하지 않습니다.
...
<Callout type="info" title="훅 인스턴스 안에서 중복 동기화 방지">
  <ul>
    <li>
      각 <code>useSyncedModel</code> 훅 인스턴스는 자체 <code>syncInProgressRef</code>로 그 인스턴스에서 겹친 <code>sync()</code> 호출을 합칩니다. 서로 다른 컴포넌트 인스턴스에서는 별도 요청이 실행될 수 있습니다.
    </li>
  </ul>
</Callout>
...
<Callout type="info" title="충돌 감지는 아직 TODO">
  <ul>
    ...
    <li>
      모델의 <code>merge</code> 옵션은 현재 데이터와 서버 replacement 데이터를 병합할 뿐, 동시 탭 쓰기를 감지하거나 해결하지 않습니다.
    </li>
  </ul>
</Callout>
...
- 데이터와 에러가 아직 없으면 훅이 `model.getSyncPromise(fetcher)`를 호출하고 반환된 Promise를 throw하여 가장 가까운 Suspense boundary가 fallback을 표시하게 합니다.
- 캐시에 데이터가 있으면 즉시 반환하고 Suspense로 빠지지 않습니다.
- 에러가 있다면 ErrorBoundary로 흘려보냅니다.

`getSyncPromise()`은 Promise를 반환할 뿐 직접 throw하지 않습니다. cache·error 상태를 확인해 Promise를 throw하는 책임은 `useSuspenseSyncedModel`에 있으므로, 직접 제어할 이유가 없다면 훅을 사용하세요.
...
- 브로드캐스트: `broadcast`, `broadcast.fallback`, `broadcast.skipped`
