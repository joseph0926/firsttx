...
Local-First is a package for building a **persistent client cache**
on top of **IndexedDB + a synchronous React store + Zod schemas**.

- Uses IndexedDB as the single source of truth
- Validates type and integrity with Zod schemas
...
<Callout type="info" title="When is Local-First a good fit?">
  <ul>
    ...
    <li>
      When you want to read persisted <strong>“last known state”</strong> on offline/unstable networks
    </li>
...
<ApiTable
  ...
  items={[
    ...
    },
    {
      ...
      description: (
        <>
          Version to bump when you make schema/shape changes that can break existing data. If the stored version differs, existing data is deleted. If <code>initialData</code> is provided, it will be used for initialization; otherwise, <code>null</code> is returned, allowing you to fetch fresh data from the server.
        </>
      ),
    },
    {
...
<Callout type="warning" title="Defending data integrity">
  <ul>
    ...
    <li>
      When you bump <code>version</code>, defining <code>initialData</code> allows a smooth transition from "broken data → new initial state". Without it, <code>null</code> is returned, allowing you to fetch fresh data from the server.
    </li>
  </ul>
</Callout>
...
<DocsAnchor id="react-hooks-usemodel-vs-usesyncedmodel" />
...
<DocsAnchor id="usemodel-when-you-only-need-local-data" />
...
- `useModel` synchronously returns the model's current **in-memory external-store snapshot**.
- IndexedDB loading happens asynchronously in the model lifecycle and updates that snapshot when it completes. The hook itself does not trigger server synchronization.
...
<DocsAnchor id="usesyncedmodel-when-you-want-server-sync-as-well" />
...
<Callout type="info" title="Avoiding duplicate sync within one hook instance">
  <ul>
    <li>
      Each <code>useSyncedModel</code> hook instance uses its own <code>syncInProgressRef</code> to deduplicate overlapping <code>sync()</code> calls from that instance. Separate component instances can still issue separate requests.
    </li>
  </ul>
</Callout>
...
<DocsAnchor id="multi-tab-sync-broadcastchannel" />
...
<Callout type="info" title="Conflict detection is still TODO">
  <ul>
    ...
    <li>
      The model&apos;s <code>merge</code> option only combines current data with incoming server replacement data. It does not detect or resolve concurrent cross-tab writes.
    </li>
  </ul>
</Callout>
...
<DocsAnchor id="suspense-integration" />
...
- When there is no data and no error yet, the hook calls `model.getSyncPromise(fetcher)` and throws the returned Promise so the nearest Suspense boundary can render its fallback.
...
`getSyncPromise()` only returns a Promise; it does not throw by itself. Prefer `useSuspenseSyncedModel` unless you also implement the cache/error checks and Promise-throwing boundary that the hook owns.

---

<DocsAnchor id="devtools-integration" />
...
- Broadcast activity: `broadcast`, `broadcast.fallback`, `broadcast.skipped`
- Validation failures: `validation.error`
...
- When validation failures were emitted
- Whether broadcasts are working (or falling back) as expected

---

<DocsAnchor id="recommended-patterns" />
