...
1. **Prepaint만 먼저 붙여서 재방문 부트 구간에 마지막 visual snapshot을 replay**하고,
...
앱의 진입점에서 기존 `ReactDOM.createRoot` 대신 `createFirstTxRoot`를 사용합니다. 진입점을 바꾸는 곳은 보통 `main.tsx`입니다.
...
<Callout type="success" title="여기까지 확인해 보세요">
  <ol>
    ...
    <li>React 앱 마운트 전에 마지막 visual snapshot이 표시되면 Prepaint가 동작하는 것입니다. 목표 기기와 네트워크 조건에서 시간을 측정하세요.</li>
  </ol>
...
2. 서버 요청이 실패하면 `compensate`가 실행되어 UI를 원래 상태로 되돌리려고 시도하며,
