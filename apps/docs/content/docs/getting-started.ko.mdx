...
export default defineConfig({
  plugins: [
    react(),
    firstTx({ overlay: true }), // 부트 스크립트 주입 + 캐시 DOM을 React 루트 밖에 유지
  ],
});
```
...
<Callout type="warning" title="현재 릴리스에서는 오버레이 모드를 사용하세요">
  legacy direct-restore 경로는 캐시된 client-rendered DOM에 하이드레이션을 시도할 수 있습니다. 이 경로는 마이그레이션과 디버깅을 위해 문서화하지만 지원되는 렌더링 계약은 아닙니다. 새 통합에서는 <code>overlay: true</code>를 유지하세요.
</Callout>
