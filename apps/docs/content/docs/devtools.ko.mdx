...
<Callout type="info" title="무엇을 디버그할 수 있나요?">
  ...
  <ul>
    <li>
      <strong>“왜 Prepaint가 복원되지 않았지?”</strong> → restore / handoff 이벤트 확인
    </li>
    <li>
...
- **한계**
  ...
  - 실제 관찰 가능한 이벤트 계약은 각 runtime package의 emitter가 소유합니다. 브리지 타입은 이를 소비하는 내부 schema이므로 구현보다 뒤처질 수 있습니다.
...
- 자체 뷰어나 로깅 시스템을 만들고 싶다면 →
  - runtime package emitter를 먼저 확인하고 `@firsttx/devtools`의 타입 정의는 계약이 아닌 통합 참고 자료로 사용하며,
