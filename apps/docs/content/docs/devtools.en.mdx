...
- Prepaint: restore/capture/handoff events
...
<Callout type="info" title="What can you debug?">
  ...
  <ul>
    <li>
      <strong>"Why didn't Prepaint restore?"</strong> → Check <code>prepaint.restore</code> and <code>prepaint.handoff</code>.
    </li>
    <li>
...
<DocsAnchor id="installation-requirements" />
...
<DocsAnchor id="event-categories" />
...
<DocsAnchor id="model-local-first" />
...
<DocsAnchor id="under-the-hood-bridge-buffering" />
...
- **Schema mismatch caveat**
  ...
  - Treat the runtime package emitters as the observable event contract. The bridge types are an internal consumer schema that may lag behind them.
...
<DocsAnchor id="performance-ux-considerations" />
...
<DocsAnchor id="programmatic-usage" />
...
- If you build your own viewer,
  - Inspect the runtime package emitters first, then use the TypeScript types in `@firsttx/devtools` as an integration aid rather than the source of truth.
