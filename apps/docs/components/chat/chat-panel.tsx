    ...
    <>
        ...
        <AnimatePresence mode="wait">
          {messages.length === 0 ? (
            <motion.div key="empty" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col items-center py-8">
              ...
              <p className="text-sm font-medium text-foreground">FirstTx에 대해 무엇이든 물어보세요!</p>
              <p className="mt-1 text-xs text-muted-foreground">문서 기반으로 답변해 드려요</p>
              <div className="mt-6 w-full space-y-2">
                <p className="text-center text-[11px] font-medium tracking-wide text-muted-foreground/70 uppercase">예시 질문</p>
                {EXAMPLE_QUESTIONS.map(({ icon: Icon, text }, i) => (
                  <motion.button key={text} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 + i * 0.05 }} onClick={() => setInput(text)} className="group flex w-full items-center gap-3 rounded-xl border border-border/50 bg-muted/30 px-4 py-3 text-left text-sm transition-all hover:border-primary/30 hover:bg-primary/5">
                    ...
                    <span className="text-foreground/80 transition-colors group-hover:text-foreground">{text}</span>
                  </motion.button>
                ))}
              </div>
            </motion.div>
      ...
      <form onSubmit={handleSubmit} className="border-t border-border/50 bg-muted/30 p-4">
        <div className="flex gap-2">
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="FirstTx에 대해 질문하세요..." className={cn("flex-1 rounded-xl px-4 py-2.5 text-sm", "border border-border/50 bg-background", "focus:border-primary/50 focus:ring-2 focus:ring-primary/20 focus:outline-none", "placeholder:text-muted-foreground/60", "transition-all")} disabled={isLoading} />
