...
export const NavbarRoot = ({ children, className, scrollContainerRef }: NavbarProps) => {
  ...
  return <motion.div className={cn("sticky inset-x-0 top-4 z-40 w-full", className)}>{React.Children.map(children, (child) => (React.isValidElement(child) ? React.cloneElement(child as React.ReactElement<{ visible?: boolean }>, { visible }) : child))}</motion.div>;
};
...
export const MobileNavMenu = ({ children, className, isOpen }: MobileNavMenuProps) => {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={cn("absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-white px-4 py-8 shadow-[0_0_24px_rgba(34,42,53,0.06),0_1px_1px_rgba(0,0,0,0.05),0_0_0_1px_rgba(34,42,53,0.04),0_0_4px_rgba(34,42,53,0.08),0_16px_68px_rgba(47,48,55,0.05),0_1px_0_rgba(255,255,255,0.1)_inset] dark:bg-neutral-950", className)}>
          {children}
        </motion.div>
      )}
    </AnimatePresence>
...
export const NavbarLogo = ({ href, src, title, subTitle }: { href: string; src: string; title: string; subTitle?: string }) => {
  return (
