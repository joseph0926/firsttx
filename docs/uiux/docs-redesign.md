...
| AC3  | 특정 답을 찾는 재방문 개발자                   | 선형 독파 없이 group, TOC와 deep link로 이동                      | navigation/anchor browser test                     | complete — grouped sidebar current와 13/13 TOC target, mobile Docs/TOC panel 검증                                                   |
...
| AC8  | Chat disabled/loading/error/unknown/rate-limit | Docs를 계속 읽고 탐색하며 복구 또는 관련 문서로 이동              | local Chat state fixture와 browser evidence        | complete — disabled 0개, empty/streaming/unknown/error/rate-limit status·alert·recovery 확인                                        |
...
| AC10 | 기존 URL 또는 검색 엔진                        | 기존 route, canonical, hreflang, sitemap과 주요 anchor가 유지     | route/metadata/sitemap tests                       | complete — 18/18 route absolute canonical과 KO/EN/x-default, 13/13 representative anchor target, build route tree 확인              |
...
| Docs navigation    | active document, expanded group, mobile sidebar, current TOC                                                    | 직접 이동과 deep link 보장                | implemented — grouped sidebar, contextual TOC와 mobile panels                                                  |
| Reading surface    | first load, long content, valid anchor, invalid route/anchor                                                    | 본문 우선, 보조 UI가 읽기 비차단          | implemented — long-form production capture, valid target와 invalid anchor/404 확인                             |
...
| Chat               | disabled, closed, empty, submitted, streaming, answered, insufficient context, network/server error, rate limit | 모든 상태에서 Docs 비차단, 추측 답변 금지 | implemented — feature-disabled hide, grounded state presentation과 recovery links                              |
...
| Legacy anchors           | heading 재작성으로 외부 deep link가 깨질 수 있음                       | 기존 anchor inventory와 compatibility test                                                                            |
| Chat state               | 외부 backend에 의존하면 재현 불가능                                    | local deterministic state fixture                                                                                     |
