...
| 시나리오                | 기존 해결책                | 문제점                          |
| ----------------------- | -------------------------- | ------------------------------- |
| 재방문 시 빈 화면       | SSR/SSG 도입               | 인프라 비용, 복잡도 증가        |
| 새로고침 시 데이터 손실 | IndexedDB 직접 구현        | 500줄 이상의 보일러플레이트     |
| 낙관적 UI 롤백          | 수동 try-catch 체인        | 에러 발생 쉬움, 유지보수 어려움 |
| 다중 탭 동기화          | BroadcastChannel 직접 구현 | 복잡한 상태 관리                |
...
│  ─────────────     │  ────────────      │  ──                   │
│  React 로드 전      │  React + IndexedDB │  원자적 트랜잭션       │
│  IndexedDB 스냅샷   │  자동 동기화        │  자동 롤백 지원        │
│                    │                    │                       │
│  결과: 재방문 시    │  결과: 새로고침해도 │  결과: 안전한          │
│  빈 화면 0ms       │  데이터 유지        │  낙관적 업데이트       │
└─────────────────────────────────────────────────────────────────┘
```

### 핵심 차별점

1. **Prepaint**: JavaScript 로드 전에 UI 복원하는 유일한 솔루션 (SSR 없이 SSR급 UX)
2. **동기 + 비동기 API**: React 렌더링용 `getCachedSnapshot()` + 비동기 작업용 `getSnapshot()`
3. **보상 패턴**: 롤백 한 번 선언하면, 실패 시 역순으로 자동 실행
4. **ViewTransition**: 롤백 시 부드러운 UI 전환을 위한 네이티브 통합
